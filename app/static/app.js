...
function courseList(items) { return items.length ? `<ul>${items.map(item => `<li><a href="${escapeAttribute(item.url)}" target="_blank" rel="noreferrer">${escapeHtml(item.title)}</a></li>`).join('')}</ul>` : '<p>No targeted course found.</p>'; }
