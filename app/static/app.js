...
function courseList(items) { return items.length ? `<ol>${items.map(item => `<li><a href="${escapeAttribute(item.url)}" target="_blank" rel="noreferrer">${escapeHtml(item.title)}</a><br><span class="mono">${escapeHtml(item.provider)}</span><br><span>${escapeHtml(item.reason)}</span></li>`).join('')}</ol>` : '<p>No targeted course found.</p>'; }
